...
def divide(source_dir,targetPath,train_percent,val_percent,test_percent,classes_dict):
    #--获取所有VOC标签的名称
    temp_xml = os.listdir(source_dir+'Annotations/')
    total_xml = []
    ...
    val_num = int(val_percent * num)
    test_num = int(test_percent * num)

    #获得全部文件索引
    # Index_All =[i for i in range(num)]
    Index_All =total_xml
    random.shuffle(Index_All)
    #获得训练集文件索引
    train_Index = Index_All[0:train_num]
    val_Index = Index_All[train_num:train_num+val_num]
    test_Index = Index_All[train_num+val_num:]
    #叠加trainval

    print("train and val size", train_num)
    print("train size", val_num)
...
if __name__ == '__main__':
    ...
    val_percent = 0.2
    test_percent = 0.1
    divide(sourcePath,targetPath,train_percent,val_percent,test_percent,classes_dict)
